 ...
 * ════════════════════
 * FIXES:
 *  - Idempotent: لا يُعلن المتغيرات مرتين إذا حُمّل الملف مرتين
 *  - Image 404 safety net: يلتقط أي <img> فاشلة ويستبدلها بالـ FALLBACK
 *  - يخفي تحذير Tailwind CDN من console
 */
...
window.Sanitize = {
    ...
    obj(data) {
        ...
        Object.entries(data).forEach(([k,v]) => {
            out[k] = typeof v === 'string' ? window.Sanitize.html(v) : v;
        });
        return out;
    }
};
...
window.Cloudinary = {
    ...
    img(src, alt='', cls='') {
        const safeSrc = this._isValidSrc(src) ? src : this.FALLBACK;
        const s = window.Sanitize.html(safeSrc);
        return `<img src="${s}" alt="${window.Sanitize.html(alt)}" class="${window.Sanitize.html(cls)}" loading="lazy" data-fallback="1">`;
    }
};
...
window.Throttle = {
    ...
    check(base) {
        ...
        if (banUntil && Date.now()<banUntil) {
            ...
            window.Toast.show(`محاولات كثيرة — محظور لـ ${m}`,'error');
            return false;
        }
        return true;
    },
...
// ═══ GLOBAL IMAGE 404 SAFETY NET ═══
// يلتقط أي <img> فاشلة أو src فيه ${...} ويستبدلها بالـ FALLBACK
document.addEventListener('error', function(e) {
    const t = e.target;
    if (t && t.tagName === 'IMG') {
        if (!window.Cloudinary._isValidSrc(t.src) || t.dataset.fallback === '1') {
            t.onerror = null;
            t.dataset.fallback = 'done';
            if (t.src !== window.Cloudinary.FALLBACK) {
                t.src = window.Cloudinary.FALLBACK;
            } else {
...
(function() {
    ...
    console.warn = function() {
        const m = arguments[0];
        if (typeof m === 'string' && m.indexOf('cdn.tailwindcss.com') > -1) return;
        _w.apply(console, arguments);
    };
})();

// ═══ BACKWARD COMPAT ALIASES (لو كود قديم يستخدم const أسماء) ═══
